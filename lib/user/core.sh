...
create_milou_user() {
    milou_log "STEP" "Creating milou user for secure operations..."
    ...
    milou_log "DEBUG" "Creating group: $MILOU_GROUP (GID: $MILOU_GID)"
        if ! groupadd -g "$MILOU_GID" "$MILOU_GROUP" 2>/dev/null; then
            # If GID is taken, let system assign one
    milou_log "DEBUG" "GID $MILOU_GID taken, letting system assign one"
    ...
    milou_log "DEBUG" "Creating docker group"
        if ! groupadd docker; then
    milou_log "WARN" "Failed to create docker group, user will be created without docker access"
            # Continue without docker group - we'll try to add it later
        fi
    fi
    
    # Create user with home directory
    milou_log "DEBUG" "Creating user: $MILOU_USER (UID: $MILOU_UID)"
    local useradd_cmd=(
        useradd
    ...
    milou_log "DEBUG" "Adding user to docker group during creation"
    fi
    
    # Try with specific UID first
    if ! "${useradd_cmd[@]}" -u "$MILOU_UID" "$MILOU_USER" 2>/dev/null; then
        # If UID is taken, let system assign one
    milou_log "DEBUG" "UID $MILOU_UID taken, letting system assign one"
    ...
    milou_log "WARN" "Docker group does not exist, user created without docker access"
    milou_log "INFO" "💡 Install Docker first, then run: sudo usermod -aG docker $MILOU_USER"
    fi
    ...
    milou_log "INFO" "Home directory: $MILOU_HOME"
    milou_log "INFO" "UID: $(id -u "$MILOU_USER"), GID: $(id -g "$MILOU_USER")"
    
    # Verify Docker access for the new user
    milou_log "DEBUG" "Verifying Docker access for newly created user..."
    ...
    milou_log "WARN" "⚠️  Docker access verification failed for $MILOU_USER user"
    milou_log "INFO" "💡 This may resolve after the first login or with proper group activation"
    ...
    milou_log "WARN" "Docker not available for verification"
    fi
}
...
validate_milou_home() {
    ...
    milou_log "ERROR" "Could not determine milou user home directory"
        return 1
    fi
    
    if [[ ! -d "$milou_home" ]]; then
    milou_log "WARN" "Milou user home directory does not exist: $milou_home"
        return 1
    fi
    
    if [[ ! -w "$milou_home" ]] && [[ "$(whoami)" == "$MILOU_USER" ]]; then
    milou_log "WARN" "Milou user cannot write to home directory: $milou_home"
        return 1
    fi
